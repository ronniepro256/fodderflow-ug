...
interface AnimalProfile {
  name: string;
  dailyIntake: number; // kg per animal per day
  count: number;
...
const UpdateStoreForm = ({ isOpen, onClose, onSubmit, animals }: UpdateStoreFormProps) => {
  ...
  const kgAdded = formData.bagsAdded * 50; // 50kg per bag
...
export default function FodderFlowDashboard() {
  ...
  const handleUpdateStore = (formData: UpdateFormData) => {
  ...
  const percentageRemaining = Math.min(100, (daysLeft / 30) * 100); // Normalize to 30-day month
